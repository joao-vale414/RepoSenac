...
INSERT INTO VENDA (
DATA_VENDA,PRODUTO,CATEGORIA,VALOR_UNITARIO,QUANTIDADE,
CLIENTE,CIDADE,ESTADOR)
VALUES
...
# 1. Liste os nomes dos produtos vendidos e seus respectivos valores unitários.

SELECT PRODUTO,VALOR_UNITARIO 
FROM VENDA;

# 2. Recupere todas as vendas realizadas na cidade de 'São Paulo’.

SELECT SUM(VALOR_UNITARIO * QUANTIDADE) AS TOTAL_VENDAS_SP
FROM VENDA 
WHERE CIDADE = 'São Paulo';

# 3. Calcule o valor total arrecadado com todas as vendas, considerando valor unitário x quantidade.

SELECT SUM(VALOR_UNITARIO * QUANTIDADE) AS TOTAL_VENDAS
FROM VENDA;

# 4. Agrupe os dados por categoria e calcule o total vendido em cada uma.

SELECT CATEGORIA, SUM(VALOR_UNITARIO * QUANTIDADE) AS TOTAL_VENDAS_CATEGORIA
FROM VENDA
GROUP BY CATEGORIA;

# 5. Identifique os clientes que compraram mais de R$3000 em produtos.

SELECT CLIENTE, SUM(VALOR_UNITARIO * QUANTIDADE) AS TOTAL_CLIENTE
FROM VENDA
GROUP BY CLIENTE
HAVING TOTAL_CLIENTE > 3000;

# 6. Liste todas as vendas realizadas na cidade de 'Curitiba’.

SELECT CIDADE,CLIENTE,PRODUTO AS VENDA_CURITIBA
FROM VENDA 
WHERE CIDADE = "Curitiba";

# 7. Liste os clientes que acumularam mais de R$4000 em compras.

SELECT CLIENTE, SUM(VALOR_UNITARIO * QUANTIDADE) AS TOTAL_COMPRAS
FROM VENDA
GROUP BY CLIENTE
HAVING TOTAL_COMPRAS > 4000;

# 8. Filtre clientes que compraram mais de R$5000. Compare o uso de WHERE e HAVING.
...
# 1.1 Quantas compras cada cliente realizou. Considere apenas vendas com valor unitário acima de R$200.
...
# 1.2 Agrupe as vendas por estado e calcule o total vendido. 
#Considere apenas vendas realizadas nas cidades de Curitiba,
#Porto Alegre e Belo Horizonte. Exiba apenas os estados cujo
#total vendido ultrapasse R$5000

SELECT ESTADOR,SUM(VALOR_UNITARIO * QUANTIDADE) AS TOTAL_VENDIDO
FROM VENDA
WHERE CIDADE IN ("Curitiba","Porto Alegre","Belo Horizonte")
GROUP BY ESTADOR
HAVING TOTAL_VENDIDO > 5000;

# 1.3 Calcule o valor médio gasto por cliente na categoria
#Informática. Exiba apenas os clientes cuja média de gasto
#seja superior a R$1000.

SELECT CLIENTE, ROUND(AVG(VALOR_UNITARIO * QUANTIDADE),2) AS MEDIA_GASTO
FROM VENDA
WHERE CATEGORIA = "Informática"
GROUP BY CLIENTE
HAVING MEDIA_GASTO > 1000;
